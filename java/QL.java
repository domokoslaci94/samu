...
public class QL
{
  ...
  public class Perceptron
  {
	...
	{
	  backs[i - 1][j] =
	    sigmoid (units[i][j]) * (1.0 - sigmoid (units[i][j])) * (y[j] -
								     units[i]
								     [j]);
      ...
      for (int i1 = n_layers - 2; i1 > 0; --i1)
	{
	  for (int j = 0; j < n_units[i1]; ++j)
	      ...
	      for (int l = 0; l < n_units[i1 + 1]; ++l)
		{
	      ...
	      backs[i1 - 1][j] =
		sigmoid (units[i1][j]) * (1.0 - sigmoid (units[i1][j])) * sum;

	      for (int k = 0; k < n_units[i1 - 1]; ++k)
		{
		  weights[i1 - 1][j][k] +=
		    (0.19 * backs[i1 - 1][j] * units[i1 - 1][k]);
		}
	    }
	}
  ...
  double max_ap_Q_sp_ap (double image[])
  {
    ...
    for (Iterator < Entry < SPOTriplet, Perceptron >> it =
	 prcps.entrySet ().iterator (); it.hasNext ();)
      {

	Entry < SPOTriplet, Perceptron > thisEntry =
	  (Entry < SPOTriplet, Perceptron >) it.next ();
  ...
  SPOTriplet operator (SPOTriplet triplet, String prg, double image[])
  {
    ...
    if (prev_reward > -java.lang.Double.MAX_VALUE
	&& frqs.get (prev_action) != null)
      {
	frqs.get (prev_action).put (prev_state,
				    frqs.get (prev_action).get (prev_state) +
				    1);
	...
	for (int z = 0; z < 10; ++z)
	  {
	    ...
	    double q_q_s_a = nn_q_s_a
	      + alpha (frqs.get (prev_action).get (prev_state)) * (reward +
								   gamma *
								   max_ap_q_sp_ap
								   -
								   nn_q_s_a);
	    ...
	    System.out.println ("### " + (q_q_s_a - nn_q_s_a) + " " +
				q_q_s_a + " " + nn_q_s_a);
	  }
